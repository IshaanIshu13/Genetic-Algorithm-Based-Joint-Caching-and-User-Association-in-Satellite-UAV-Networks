...
# last_best_fitness = 1000
# #print("initial best", last_best_fitness)
# a2 = -1 + iter * ((-1) / I_max)
# # Main WOA loop
# while iter < I_max:
#   t = random.random()  # Random value in [0, 1]
#   y = random.random()  # Random value in [0, 1]
#   q = 2 * random.random() - 1  # Random value in [-1, 1]
#   l = (a2 - 1) * random.random() + 1
#   for k in range(W):
#       # Store whale's initial position before the update
#       #initial_position = whales[k]['position'].copy()
#       b = 2 * (1 - iter / I_max) # Decreasing vector from 2 to 0
#       G = 2 * b * y - b  # Random value in [0, 2]
#       J = 2 * y
#       pBWOA = random.random()  # Random value in [0, 1]
#       if t < 0.5:
#           if abs(G) < 1:
...
#           if(pBWOA < sigma_sup.all()):
#            whales[k]['position'] = (1 - whales[k]['position'])
#       #Usage in your code
#       whales[k]['position'] = enforce_roh_constraints_after_toggle(whales[k]['position'], Nu, U, user_pos, uav_pos, uavs_per_region, user_uav_data_rates)
#       # Count the total number of 1's in the roh matrix
#       whales[k]['fitness'] = calculate_fitness_for_whale(
#           whales[k]['position'], N, F, Gamma, clusters,
#           user_uav_data_rates, uav_uav_data_rates,
#           sat_uav_data_rates, file_size, tau, phi
#       )
...
#     #print("condition hit")
#     best_whale['position'] = new_best_whale['position']  # Update the best whale
#     best_whale['fitness'] = new_best_whale['fitness']
...
#   # print("Total number of 1's in new_best_whale['position']:", total_ones)
#   print(f"Iteration {iter}, Best Fitness Last: {last_best_fitness}")
#   #print(f"Iteration {iter}, Best Fitness Last: {best_whale['fitness']}")
#   iter += 1
...
# print("roh", optimized_rho)
# #print("optimizes service cost", Scost)
# #print("cache", Gamma)
# # Compare the minimum fitness value of this iteration's population to the minimum fitness from the previous iterations.
# # If the minimum fitness value of the current iteration is better (lower) than the previous best, update the best whale to this new position and fitness.
...
# Function to initialize all particles for DPSO
def initialize_particles(num_particles, N, U, Nu, user_pos, uav_pos, uavs_per_region):
    particles = []
    for _ in range(num_particles):
        # Initialize an empty UAV-user association matrix (roh)
        roh = np.zeros((N, U))

        # Use the same logic as before to get a valid starting position
        roh = initialize_primary_uav(roh, user_pos, uav_pos, Nu, U, uavs_per_region)
        
        # Initialize velocity matrix with small random values
        velocity = np.random.rand(N, U) * 0.1 # Small initial probabilities

        # Each particle stores its position, velocity, fitness, best position, and best fitness
        particles.append({
            'position': roh,
            'velocity': velocity,
            'fitness': float('inf'),
            'pbest_position': roh.copy(), # Personal best position is the initial position
...
def calculate_fitness_for_particle(roh, N, F, Gamma, clusters, user_uav_data_rates, uav_uav_data_rates, sat_uav_data_rates, file_size, tau, phi):
    # Calculate transmission delay and energy consumption for this whale
    total_delay, total_energy = assign_secondary_and_calculate(N, F, roh, Gamma, clusters, user_uav_data_rates, uav_uav_data_rates, sat_uav_data_rates, file_size, tau, phi)
    # print("\nTotal Delay in sec for each user:")
    # print(total_delay)
    #print("\nEnergy Consumption (Joules) for each user:")
    #print(total_energy)
...
particles = initialize_particles(num_particles, N, U, Nu, user_pos, uav_pos, uavs_per_region)

# Initialize global best
gbest_position = np.zeros((N, U))
gbest_fitness = float('inf')
# 2. Main DPSO loop
for iter in range(I_max):
    for particle in particles:
        # a. Calculate fitness for the particle
        particle['fitness'] = calculate_fitness_for_particle(
            particle['position'], N, F, Gamma, clusters,
            user_uav_data_rates, uav_uav_data_rates,
            sat_uav_data_rates, file_size, tau, phi
        )

        # b. Update personal best (pbest)
        if particle['fitness'] < particle['pbest_fitness']:
            particle['pbest_fitness'] = particle['fitness']
            particle['pbest_position'] = particle['position'].copy()

        # c. Update global best (gbest)
        if particle['fitness'] < gbest_fitness:
            gbest_fitness = particle['fitness']
            gbest_position = particle['position'].copy()

    # d. Update particle velocity and position
    for particle in particles:
        # Random numbers for stochastic update
        r1 = np.random.rand(N, U)
        r2 = np.random.rand(N, U)
        ...
        # Sigmoid function to squash velocity into a probability (0 to 1) : Used for Velocity Scaling
        s_v = 1 / (1 + np.exp(-particle['velocity']))
        
        # Update position based on the probability
        # For each element in the roh matrix, if a random number is less than the probability, flip the bit
        # Why use random matrix? To ensure that the update is stochastic and allows exploration of the search space
        # Generate a random matrix of the same shape as roh
        # This will determine which bits to flip based on the velocity probabilities
        random_matrix = np.random.rand(N, U)
        new_position = particle['position'].copy()
        flip_mask = random_matrix < s_v
        new_position[flip_mask] = 1 - new_position[flip_mask] # Toggle 0s and 1s

        # Enforce constraints to make sure the new position is a valid solution
        particle['position'] = enforce_roh_constraints_after_toggle(
            new_position, Nu, U, user_pos, uav_pos, uavs_per_region, user_uav_data_rates
        )

    print(f"Iteration {iter + 1}/{I_max}, Best Fitness (Service Cost): {gbest_fitness}")

# --- End of DPSO Loop ---

# Output the best solution found
print("\n--- Optimized Results from DPSO ---")
print("Optimized Service Cost:", gbest_fitness)
print("Optimized roh matrix (User-UAV assignments):")
