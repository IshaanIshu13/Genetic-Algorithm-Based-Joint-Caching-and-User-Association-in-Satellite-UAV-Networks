...
# LETS CHANGE THIS TO GENTIC ALGORITHM 

# Function to initialize all chromosomes (individuals in GA)
def initialize_population(pop_size, N, U, Nu, user_pos, uav_pos, uavs_per_region):
    population = []
    for _ in range(pop_size):
        roh = np.zeros((N, U))
        roh = initialize_primary_uav(roh, user_pos, uav_pos, Nu, U, uavs_per_region)
        population.append({'position': roh, 'fitness': None})
    return population

# Tournament selection of the offspirng
...
def crossover(parent1, parent2):
    child = {'position': np.copy(parent1['position']), 'fitness': None}
    rows, cols = child['position'].shape
    point = random.randint(0, rows - 1)
    child['position'][point:] = parent2['position'][point:]
    return child

# Mutation: flip a random bit in roh matrix
def mutate(individual, mutation_rate=0.05):
    roh = np.copy(individual['position'])
    N, U = roh.shape
    for i in range(N):
        for j in range(U):
            if random.random() < mutation_rate:
                roh[i][j] = 1 - roh[i][j]
    individual['position'] = roh
    return individual

Gamma = Gamma1

# Fitness function (unchanged)
def calculate_fitness_for_genetic(roh, N, F, Gamma, clusters, user_uav_data_rates, uav_uav_data_rates, sat_uav_data_rates, file_size, tau, phi):
...
population = initialize_population(pop_size, N, U, Nu, user_pos, uav_pos, uavs_per_region)

# Calculate fitness for each individual
for individual in population:
    individual['fitness'] = calculate_fitness_for_genetic(individual['position'], N, F, Gamma, clusters, user_uav_data_rates, uav_uav_data_rates, sat_uav_data_rates, file_size, tau, phi)
...
while gen < I_max:
    ...
    for i in range(len(children)):
        children[i] = mutate(children[i], mutation_rate=0.05)
        children[i]['position'] = enforce_roh_constraints_after_toggle(children[i]['position'], Nu, U, user_pos, uav_pos, uavs_per_region, user_uav_data_rates)
        children[i]['fitness'] = calculate_fitness_for_genetic(children[i]['position'], N, F, Gamma, clusters, user_uav_data_rates, uav_uav_data_rates, sat_uav_data_rates, file_size, tau, phi)

    # New generation
    population = children

    # Update best individual
    new_best = min(population, key=lambda x: x['fitness'])
    if new_best['fitness'] < last_best_fitness:
        best_individual = new_best
        last_best_fitness = new_best['fitness']

    print(f"Generation {gen}, Best Fitness: {last_best_fitness}")
    gen += 1

# Final output
optimized_rho = best_individual['position']
